...
const BackupModal = ({ open, onClose }) => {
  ...
  const handleBackup = async () => {
    ...
    const url = `/backup/backup?start=${startDate}&end=${endDate}`;
    await handleDownload(
      url,
      `marketing_backup_${startDate}_to_${endDate}.zip`
    );
  };
  ...
  const handleFullBackup = async () => {
    ...
    await handleDownload(
      url,
      `marketing_full_backup_${dayjs().format("YYYY-MM-DD")}.zip`
    );
  };
  ...
  const handleDeleteByDate = async () => {
    ...
    try {
      ...
      const response = await customFetch.delete(
        `/backup/delete/?start=${startDate}&end=${endDate}`
      );
      showMessage(
        response.data.message || "Images deleted successfully.",
        true
      );
    } catch (error) {
  ...
  const handleFullDelete = async () => {
    try {
      ...
      showMessage(
        response.data.message || "All images deleted successfully.",
        true
      );
    } catch (error) {
      console.error("Delete error:", error);
  ...
  return (
    <>
              ...
              <Card variant="outlined">
                <CardContent>
                  ...
                  <Typography variant="body2" color="text.secondary" paragraph>
                    Perform operations on all marketing logs in the system.
                  </Typography>
                </CardContent>
                <CardActions
                  sx={{
                ...
                >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        openConfirmDialog(
                          "backupAll",
                          "Full Backup",
                          "Download all images from the system? This may take time for large datasets."
                        )
                      }
                      disabled={loading}
                  ...
                  <Tooltip title="Delete all images permanently">
                    <Button
                      variant="contained"
                      color="error"
                      onClick={() =>
      ...
      {/* Simple confirm for backup by date */}
